function bname -d "Generates a Git branch name using a Jira Ticket ID"
    ...
    set -l issue_scope_and_summary (echo $raw_issue_data | jq -r '.fields.summary' | sed 's/"//g; s/’//g; s/\'//g; s/\+/and/g; s/,//g;' | head -c 110)
    ...
    if test $num_colons = 0
        ...
        set issue_scope (echo $issue_scope_and_summary | tr '/' '-' | cut -d ':' -f1 | tr '[:space:]' '-' | tr ':' '-' | sed 's/--/-/g; s/-$//; s/(//g; s/)//g;' | tr a-z A-Z)
